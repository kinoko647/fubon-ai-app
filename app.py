...
if check_password():
    ...
    st.markdown("""
        ...
        .ai-diag-box {
            ...
            margin-top: 25px;
            box-shadow: inset 0 0 20px rgba(255, 77, 77, 0.2);
        }
    ...
    def analyze_master_terminal(df, budget, strategy_mode):
        ...
        is_bullish_aligned = df['EMA8'].iloc[-1] > df['MA20'].iloc[-1]
        
        # 2. MACD 動能金叉 (Hist 負轉正)         df['MACD'] = df['Close'].ewm(span=12, adjust=False).mean() - df['Close'].ewm(span=26, adjust=False).mean()
        ...
        if len(all_pts_idx) >= 4:
            ...
            if v[0] > v[1] and v[2] > v[1] and v[2] > v[3]: # M頭                 if v[2] <= v[0] * 1.015:
                    ...
                    ai_win_score -= 20
                    diag_reasons.append("🔴 警示：偵測到 M 頭形態，高位套牢壓力沉重。")
            elif v[0] < v[1] and v[2] < v[1] and v[2] < v[3]: # W底                 if v[2] >= v[0] * 0.985:
                    pattern_label = "收斂 W 底 (底部起漲)"
                    ai_win_score += 35
                    diag_reasons.append("🟢 驚喜：偵測到收斂 W 底，第二次回測不破底，具備暴力噴發動能。")

        # --- [D] 診斷加權與黃金交叉 ---
        if ma_gc: 
            ai_win_score += 10
            diag_reasons.append("🟢 ✨ 黃金交叉確認：T 線正式上穿月線，多頭翻轉確立。")
        elif is_bullish_aligned:
            diag_reasons.append("🟢 趨勢多頭：目前處於健康的多頭排列。")
        else:
            diag_reasons.append("🔴 趨勢受阻：目前受壓於月線 MA20 之下。")
            
        if macd_gc:
            ai_win_score += 10
            diag_reasons.append("🟢 🚀 動能金叉：MACD 能量柱翻正，噴發動能啟動。")
        
        if curr_bandwidth < 0.12:
            ai_win_score += 15
            diag_reasons.append("🟢 💎 極致收斂：波動極度壓縮，迎接變盤大噴發。")

        # --- [E] 斐波那契全位階與試算 ---
        lookback = 120
        max_v = float(high_prices[-lookback:].max())
        min_v = float(low_prices[-lookback:].min())
        p_diff = max_v - min_v
        fib_buy = max_v - 0.618 * p_diff
        fib_target = min_v + 1.272 * p_diff
        
        # RSI 與 CCI         delta = df['Close'].diff()
        ...
        # 入場時機建議
        entry_timing = "⏳ 等待動能共振"
        if ma_gc and macd_gc: entry_timing = "🔥 即刻錄場 (雙重確認)"
        elif curr_p <= fib_buy * 1.01: entry_timing = "💎 分批低吸 (價值支撐區)"
        ...
        return {
            ...
            "gc_ma": ma_gc, "gc_macd": macd_gc, "aligned": is_bullish_aligned,
    ...
    with st.sidebar:
        ...
        st.divider()
        st.write("🔍 **全台股 500 檔海量形態偵測**")
        scan_grp = st.radio("掃描標的類別", ("權值 0050 組", "中型 0051 組", "高股息/綜合 300檔"))
        ...
        if st.button("🚀 啟動全市場自動篩選"):
            ...
            for i, code in enumerate(target_list):
                status_text.text(f"分析中: {code}.TW")
                try:
                    ...
                    if s_res and ("W底" in s_res['pattern'] or s_res['score'] >= 85):
                        scan_results.append({
                            "代碼": code, "偵測形態": s_res['pattern'], "AI勝率": f"{s_res['score']}%", "回報": f"{s_res['roi']:.1f}%"
                        })
                except: continue
                p_bar.progress((i + 1) / len(target_list))
            
            st.session_state.full_scan_data = pd.DataFrame(scan_results)
            status_text.success(f"✅ 全市場掃描完成！發現 {len(scan_results)} 支優質形態標的。")
            
        if 'full_scan_data' in st.session_state:
            st.write(f"### {scan_grp} 診斷結果")
            st.dataframe(st.session_state.full_scan_data, use_container_width=True, height=500)
    ...
    with t_col1: m_env = st.radio("當前市場環境", ("台股", "美股"), index=0 if st.session_state.m_type == '台股' else 1, horizontal=True)
    with t_col2: u_id = st.text_input("🔍 代碼深度分析", value=st.session_state.u_code)
    with t_col3: u_inv = st.number_input("💰 投資預算金額 (元)", value=1000000)
    ...
    try:
        raw_df = yf.download(ticker_final, interval=tf_map_m[st.session_state.tf_choice], period=p_map_m[st.session_state.tf_choice], progress=False)
        final_res = analyze_master_terminal(raw_df, u_inv, st.session_state.strategy)
        
        if final_res:
            # --- [A] 傑克看板 (超高對比無視角) ---
            bw_val = final_res['bw']
            bw_desc = "📉 強烈收斂 (變盤大噴發在即)" if bw_val < 0.12 else ("📊 趨勢發散" if bw_val > 0.25 else "穩定波動")
            ...
            st.markdown(f"""
                <div class="jack-panel">
                    ...
                    <p class="jack-sub-text">🔥 形態偵測：<span class="jack-status-highlight">{final_res['pattern']}</span> | <span style='color:#ffff00;'>{gc_msg}</span></p>
                    ...
                    <p class="jack-sub-text">建議佈局：<span class="jack-value">${final_res['fib_buy']:,.2f}</span> | 目標預測：<span class="jack-value">${final_res['fib_target']:,.2f}</span></p>
                </div>
            """, unsafe_allow_html=True)
            ...
            with adv_c1:
                if final_res['left_ok']: st.markdown('<div class="advice-card left-side">💎 左側訊號：價格進入 0.618 價值區，適合分批低吸。</div>', unsafe_allow_html=True)
            ...
            # --- [C] AI 為什麼勝率低？深度技術診斷 ---
            with st.expander("🔍 AI 深度診斷報告 (字體顏色已極致強化)", expanded=(final_res['score'] < 75)):
            ...
            dash3.metric("建議持有股數", f"{final_res['shares']:,} 股")
            dash4.metric("預計總獲利金額", f"${final_res['profit']:,.0f}")

            # --- [E] 📈 專業三層聯動圖表 (終極座標物理鎖定) ---
            # 此處解決成交量拉平 K 線的終極方案：強制鎖定 Y 軸對焦
        ...
        else: st.warning("系統數據解析中，請確認代碼貼上完整並稍候...")
