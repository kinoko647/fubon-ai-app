...
if check_password():
    # 🎨 極限高對比 CSS (解決字體看不清、截圖錯誤問題)
    st.markdown("""
        <style>
        .main { background-color: #0d1117; }
        [data-testid="stMetricValue"] { color: #00ffcc !important; font-weight: 900 !important; font-size: 3.5rem !important; text-shadow: 2px 2px 20px rgba(0, 255, 204, 0.7); }
        ...
        .jack-status-highlight { color: #00ffcc !important; font-weight: 900 !important; font-size: 34px; text-decoration: underline; }
        ...
        .advice-card { padding: 40px; border-radius: 20px; margin-bottom: 30px; font-weight: 900; text-align: center; border: 6px solid; font-size: 28px; }
    ...
    # ==============================================================================
    # 3. 核心雙軌引擎 (蝴蝶 XABCD + 右側突破偵測)
    # ==============================================================================
    def analyze_master_terminal(df, budget):
        if df is None or df.empty or len(df) < 60: return None
        # 徹底處理 yfinance 多層索引防止 KeyError
        ...
        # --- 雙軌偵測算法 ---
        # 1. 右軌突破：突破 20 日高點 + 布林帶擴張 (抓 2486/6188 漲停核心)
        local_max_20 = float(high_p[-20:-1].max())
        is_breakout = curr_p > local_max_20 and df['BW'].iloc[-1] > df['BW'].iloc[-2]
        
        # 2. 蝴蝶 XABCD 左側形態         n_v=10
        max_pk, min_pk = argrelextrema(high_p, np.greater, order=n_v)[0], argrelextrema(low_p, np.less, order=n_v)[0]
        ...
        if len(pts_idx) >= 4:
            ...
            if v_v[0] > v_v[1] and v_v[2] > v_v[1] and v_v[2] > v_v[3] and v_v[2] <= v_v[0]*1.015: 
                ...
                diag.append("🔴 左側警示：蝴蝶 D 點遭遇壓力，暫避高點。")
        ...
        if is_breakout: ai_score += 25; diag.append("🔥 右側突破：強勢突破 20 日高點，黑馬連噴啟動。")
        if df['EMA8'].iloc[-1] > df['MA20'].iloc[-1]: ai_score += 10; diag.append("✨ 趨勢金叉：短期力量正式轉多。")
    ...
    # ==============================================================================
    # 4. PC 側邊欄：海量 500 檔搜尋引擎 (解決 6188 找不到問題)
    # ==============================================================================
    with st.sidebar:
        ...
        scan_grp = st.radio("掃描分組", ("龍頭 0050", "中型 0051", "飆股/熱門 400檔"))
        
        if st.button("🚀 啟動全台股海量偵測"):
            ...
            for i, code in enumerate(targets):
                ...
                try:
                    # 修復 6188 等上櫃股後綴問題                     suffix = ".TWO" if code in ["6188","8046","6415","3260","3105"] else ".TW"
    ...
    # ==============================================================================
    # 5. PC 主畫面：實戰獲利計算與圖表 (徹底解決 K 線變平問題)
    # ==============================================================================
    st.title(f"🏆 股票預測分析系統 - {st.session_state.strategy} 旗艦完全體")
    ci1, ci2, ci3 = st.columns([1, 1, 1])
    with ci1: m_env = st.radio("市場環境", ("台股", "美股"), index=0 if st.session_state.m_type == '台股' else 1, horizontal=True)
    with ci2: u_id = st.text_input("🔍 代碼診斷 (例: 6188, 2486)", value=st.session_state.u_code)
    with ci3: u_inv = st.number_input("💰 投資總預算", value=1000000)
    st.session_state.u_code, st.session_state.m_type = u_id, m_env
    # 智慧補全修復     tk_f = f"{u_id}.TWO" if u_id in ["6188","8046","6415"] else f"{u_id}.TW"
    ...
    try:
        ...
        if f_res:
            ...
            with cc3: 
                ...
                roi_f = ((f_res['fib_target']/my_buy_p)-1)*100
                st.write(f"**預計獲利金額：**\n\n<span style='color:#ff3e3e; font-size:32px; font-weight:900;'>${prof:,.0f}</span> ({roi_f:.1f}%)", unsafe_allow_html=True)
            ...
            fig.add_trace(go.Scatter(x=f_res['df'].index, y=f_res['df']['EMA8'], line=dict(color='#ffff00', width=3), name='EMA8'), row=1, col=1)
            ...
            fig.update_yaxes(range=[y_min_f, y_max_f], row=1, col=1, autorange=False) # 鎖定 Y 軸
