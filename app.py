...
# ==============================================================================
# 1. 系統全局配置 (針對 PC 專業大螢幕進行極限視覺優化)
# ==============================================================================
st.set_page_config(layout="wide", page_title="2026 戰神終極終端 - PC 旗艦完全體", initial_sidebar_state="expanded")
...
def check_password():
    ...
    st.markdown("<h2 style='color:#00ffcc; text-align:center;'>解鎖蝴蝶 XABCD、雙軌偵測與 500 檔海量掃描</h2>", unsafe_allow_html=True)
...
if check_password():
    ...
    st.markdown("""
        ...
        .jack-sub-text { color: #ffffff !important; font-size: 24px !important; line-height: 2.2 !important; font-weight: 900 !important; }
        ...
        .stButton>button { border-radius: 15px; font-weight: 900; height: 5.5rem; background-color: #161b22; color: #00ffcc; font-size: 20px; border: 3px solid #00ffcc; }
        </style>
        """, unsafe_allow_html=True)

    # ==============================================================================
    # 3. 核心雙軌引擎 (蝴蝶 XABCD + 右側突破偵測 + 物理對焦)
    # ==============================================================================
    def analyze_master_terminal(df, budget):
        ...
        # 蝴蝶與 W/M 形態偵測         if len(pts_idx) >= 4:
            ...
            if v_v[0] > v_v[1] and v_v[2] > v_v[1] and v_v[2] > v_v[3] and v_v[2] <= v_v[0]*1.015: 
                ...
                diag.append("🔴 左側診斷：波段 D 點遭遇壓力。")
            elif v_v[0] < v_v[1] and v_v[2] < v_v[1] and v_v[2] < v_v[3] and v_v[2] >= v_v[0]*0.985:
                pattern_label, ai_score = "蝴蝶 W 底 (低位佈局)", ai_score+35
                diag.append("🟢 左側診斷：波段 D 點守穩支撐，價值買點。")

        if is_breakout: ai_score += 25; diag.append("🔥 右側突破：強勢突破 20 日高點，黑馬漲停基因啟動。")
    ...
    # ==============================================================================
    # 4. PC 側邊欄：海量 500 檔搜尋引擎
    # ==============================================================================
    with st.sidebar:
        st.header("⚙️ 500 檔全市場操盤終端")
        st.session_state.strategy = st.selectbox("🎯 交易戰略", ("🛡️ 長線穩健", "⚡ 中線進攻", "🔥 短線當沖"))
        ...
        if st.button("🚀 啟動全市場海量偵測"):
            ...
            c_hot = ["6188","2486","2330","2317","2454","2382","2603","2609","2615","2303","3231","2353","2376","2383","2449","3037","3034","3035","3443","3661","6669","8046","1513","1519","1503","1504","1722","1723","2881","2882","2891","5871","9921","1402","1101","1301","1303","1605","2002","2327","2357","2395","2409","2474","2801","2883","2887","2889","2890","2912","3008","3017","3045","3481","3711","4904","4938","5880","6239","6415","8215","9910"]
            ...
            for i, code in enumerate(targets):
                ...
                try:
                    # 智慧辨識上市/上櫃
                    suffix = ".TWO" if code in ["6188","8046","6415","3105"] else ".TW"
                    ...
                    if res and (res['score'] >= 85 or res['breakout']):
                        scan_l.append({"代碼": code, "形態": res['pattern'], "勝率": f"{res['score']}%", "ROI預期": f"{res['roi']:.1f}%"})
                except: continue
                bar.progress((i + 1) / len(targets))
            st.session_state.scan_res_df = pd.DataFrame(scan_l)
    ...
    # 智慧字尾辨識修復
    tk_f = f"{u_id}.TWO" if u_id in ["6188","8046","6415"] else f"{u_id}.TW"
    ...
    try:
        ...
        if f_res:
            ...
            # --- [B] 傑克看板 ---
            bw_v = f_res['bw']
            bw_desc = "📉 強烈收斂 (波動極限壓縮)" if bw_v < 0.12 else ("📊 發散趨勢" if bw_v > 0.25 else "穩定震盪")
            st.markdown(f"""
            ...
            # 雙軌標記             fig.add_trace(go.Scatter(x=[f_res['df'].index[-1]], y=[f_res['fib_buy']], mode='markers+text', name='左軌抄底', marker=dict(symbol='triangle-up', size=20, color='#ffa500'), text=['左軌'], textposition='bottom center'), row=1, col=1)
            if f_res['breakout']:
