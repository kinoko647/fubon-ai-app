...
st.markdown(f'<p style="color:#8b949e; font-size:14px;">推薦符合「{strategy}」模式標的：</p>', unsafe_allow_html=True)
...
try:
    ...
    if res:
        # 技術總結
        bw_txt = "收斂 (蓄勢)" if res['bw'] < 0.15 else "發散 (擴張)"
        div_txt = "✅ 傑克底背離 (反轉預警)" if res['div'] else "能量穩定"
        st.markdown(f'<div class="status-box">📊 傑克指標：{bw_txt} | {div_txt}</div>', unsafe_allow_html=True)
