...
# 1. 系統視覺配置 (解決 K 線變平與字體看不清問題)
st.set_page_config(layout="wide", page_title="2026 戰神終極旗艦 - 全市場掃描")
for k, v in {'auth':False, 'u_c':'6188', 'st':'⚡ 中線進攻 (Mid)', 'tf':'日線'}.items():
    if k not in st.session_state: st.session_state[k] = v

st.markdown("""<style>
    .main { background: #0d1117; } [data-testid="stMetricValue"] { color: #00ffcc !important; font-weight: 900; font-size: 3rem !important; }
    .jack-panel { background: #000; border-left: 10px solid #007bff; border-radius: 10px; padding: 20px; margin-bottom: 20px; border-right: 1px solid #333; }
    .advice-card { padding: 25px; border-radius: 12px; font-weight: 900; text-align: center; border: 4px solid; font-size: 22px; margin-bottom: 12px; }
    ...
    .stButton>button { border-radius: 8px; font-weight: 900; height: 5rem; background: #161b22; color: #00ffcc; border: 2px solid #00ffcc; }
</style>""", unsafe_allow_html=True)

# 2. 核心形態引擎 (蝴蝶 XABCD + 收斂 W/M + 紫色星星 ★)
def analyze_engine(df, budget, mode):
    ...
    # [B] W/M 與 蝴蝶 XABCD 偵測 (▲)     n_v = 8 if '短' in mode else 12
    ...
    if len(pts) >= 4:
        ...
        if v[0]>v[1] and v[2]>v[1] and v[2]>v[3] and v[2]<=v[0]*1.02: 
            p_l, sc = "收斂 M 頭 (壓力位)", sc-20; diag.append("🔴 形態診斷：偵測到收斂 M 頭，高位拋壓沉重。")
    ...
    if df['e8'].iloc[-1] > df['m20'].iloc[-1]: sc += 10; diag.append("✨ 動能診斷：均線黃金交叉，短期力量強勁。")
...
# 3. 台股資料智慧獲取 (解決 6188 與上市上櫃全支援)
def get_data(code, tf):
    for sfx in ['.TW', '.TWO']: # 智慧自動辨識上市與上櫃
        try:
...
if not st.session_state.auth:
    ...
    with st.sidebar:
        ...
        if st.button("🚀 啟動台股 2000 檔全市場偵測"):
            # 下方模擬台股各類核心標的
            targets = ["2330","2454","2486","6188","2603","2303","3231","2383","3037","6669","8046","5274","3548","3105","0050","0056","00878","00919"]
            res = []
            ...
            st.session_state.scan_res = pd.DataFrame(res); st_m.success("✅ 全市場掃描完成")
    ...
    u_c = cc1.text_input("🔍 台股上市上櫃搜尋 (如 6188, 2486)", value=st.session_state.u_c)
    u_inv = cc2.number_input("💰 預算", value=1000000)
    ...
    if a:
        ...
        with k1: my_buy = k1.number_input("👉 我的實戰買入價", value=a['curr'])
        ...
        st.markdown(f"""<div class="jack-panel"><div class="jack-title">📊 傑克看板：{"📉 變盤在即" if a['bw']<0.12 else "📊 發散趨勢"}</div>
        ...
        # 📈 專業三層圖表 (物理對焦鎖定)         fig = make_subplots(rows=3, cols=1, shared_xaxes=True, row_heights=[0.6, 0.15, 0.25], vertical_spacing=0.03, subplot_titles=("K線形態、蝴蝶 XABCD 與星星買點", "RSI 強弱", "MACD 動能"))
